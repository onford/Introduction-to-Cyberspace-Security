...
if __name__ == '__main__':
	...
	# 根据伪随机数，对 dct 后的图像进行 arnold 加密
	secreted_img = img_transform.arnold_trans(dct_img,a,b,k)
	util.show_img(secreted_img,"加密后图像")

	# 根据伪随机数，对加密后的图象进行解密（arnold 逆变换以及 idct）
	dearnold_img = img_transform.arnold_detrans(secreted_img,a,b,k) # arnold 逆变换
	...
	util.show_img(img_transform.idct_img(img_transform.arnold_detrans(img, a, b, k)),"攻击后还原的图像")